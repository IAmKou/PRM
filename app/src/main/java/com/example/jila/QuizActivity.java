...
//        if (quizId == null) {
//            Log.e("QuizActivity", "Invalid quiz ID");
//            return;
//        }
        String quizId = "oH4sUjoOfnpanOouOMG8";
            ...
            questionRef.whereEqualTo("quizId", db.document("quiz/" + quizId)).get().addOnCompleteListener(task ->{
                if (task.isSuccessful()) {
                    for (QueryDocumentSnapshot document : task.getResult()) {
                        Long questionId = Long.valueOf(document.getId());
    ...
    private void getAnswerForQuestion(Long questionId,String questionText){
        ...
        answerRef.whereEqualTo("questionId",db.document("question/" + questionId)).get().addOnCompleteListener(task ->{
            if (task.isSuccessful()) {
                ...
                for (QueryDocumentSnapshot document : task.getResult()) {
                    Long answerId = Long.valueOf(document.getId());
                ...
                questionList.add(new Question(questionText, questionId, answers));
